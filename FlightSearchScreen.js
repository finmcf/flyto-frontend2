...
export default function FlightSearchScreen(props) {
  ...
  const singleFlightSearch = () => {
    ...
    fetch(
      `http://192.168.1.101:4000/flight-search?originCode=${departureLocationIata}&destinationCode=${arrivalLocationIata}&dateOfDeparture=${departureDate}${
  ...
  const everyWhereFlightSearch = () => {
    ...
    Promise.all(
      routes.map((route) =>
        fetch(
          `http://192.168.1.101/flight-search?originCode=${
  ...
  const fetchRoutes = async (airportCode) => {
    const response = await fetch(
      `http://192.168.1.101/airport-route?departureAirportCode=${airportCode}`,
      {
        headers: {
          "Content-Type": "application/json",
          "x-api-key": apiKey,
